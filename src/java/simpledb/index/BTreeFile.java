...
public class BTreeFile implements DbFile {
    ...
    /**
     * Returns an ID uniquely identifying this BTreeFile. Implementation note: you
     * will need to generate this tableid somewhere and ensure that each BTreeFile
     * has a "unique id," and that you always return the same value for a particular
     * BTreeFile. We suggest hashing the absolute file name of the file underlying
     * the BTreeFile, i.e. f.getAbsoluteFile().hashCode().
     *
    ...
    public Page readPage(PageId pid) {
        ...
        try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(f))) {
            if (id.pgcateg() == BTreePageId.ROOT_PTR) {
                ...
                if (bis.skip(BTreeRootPtrPage.getPageSize() + (long) (id.getPageNumber() - 1)
                        * BufferPool.getPageSize()) != BTreeRootPtrPage.getPageSize()
                ...
                if (retval < BufferPool.getPageSize()) {
                    throw new IllegalArgumentException(
                            "Unable to read " + BufferPool.getPageSize() + " bytes from BTreeFile");
                }
                Debug.log(1, "BTreeFile.readPage: read page %d", id.getPageNumber());
    ...
    /**
     * Write a page to disk. This should not be called directly but should be called
     * from the BufferPool when pages are flushed to disk
     *
     ...
     * corresponding to the left-most page possibly containing the key field f. It
     * locks all internal nodes along the path to the leaf node with READ_ONLY
     ...
     * parent node as needed to accommodate a new entry. The new entry should have a
     * key matching the key field of the first tuple in the right-hand page (the key
     * is "copied up"), and child pointers pointing to the two leaf pages resulting
     * from the split. Update sibling pointers and parent pointers as needed.
     * <p>
...
//            newPage.insertTuple(tuples.get(i));
     ...
     * parent page as needed to accommodate a new entry. The new entry for the
     * parent should have a key matching the middle key in the original internal
     * page being split (this key is "pushed up" to the parent). The child pointers
     * of the new parent entry should point to the two internal pages resulting from
     * the split. Update parent pointers as needed.
     * <p>
     ...
     * new entries. This may mean creating a page to become the new root of the
     * tree, splitting the existing parent page if there are no empty slots, or
     ...
     */
    private BTreeInternalPage getParentWithEmptySlots(TransactionId tid,
            Map<PageId, Page> dirtypages, BTreePageId parentId, Field field)
            throws DbException, IOException, TransactionAbortedException {
     ...
     * @throws TransactionAbortedException
     */
    private void updateParentPointer(TransactionId tid, Map<PageId, Page> dirtypages,
            BTreePageId pid, BTreePageId child) throws DbException, TransactionAbortedException {
    ...
    private void updateParentPointers(TransactionId tid, Map<PageId, Page> dirtypages,
            BTreeInternalPage page) throws DbException, TransactionAbortedException {
     ...
     * requested page there, it fetches it from the buffer pool. It also adds pages
     * to the dirtypages cache if they are fetched with read-write permission, since
     ...
     * pages are accessed multiple times.
     *
     * @param tid        - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all
     *                   new dirty pages
    ...
    /**
     * Insert a tuple into this BTreeFile, keeping the tuples in sorted order. May
     * cause pages to split if the page where tuple t belongs is full.
     *
     * @param tid - the transaction id
    ...
    public List<Page> insertTuple(TransactionId tid, Tuple t)
        ...
        if (rootId == null) { // the root has just been created, so set the root pointer to point to
                              // it
    ...
    private void handleMinOccupancyPage(TransactionId tid, Map<PageId, Page> dirtypages,
            BTreePage page) throws DbException, IOException, TransactionAbortedException {
        BTreePageId parentId = page.getParentId();
        ...
        if (page.getId().pgcateg() == BTreePageId.LEAF) {
            ...
            handleMinOccupancyInternalPage(tid, dirtypages, (BTreeInternalPage) page, parent,
     ...
     * tuples. Otherwise merge with one of the siblings. Update pointers as needed.
     *
     * @param tid        - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all
     *                   new dirty pages
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int pageCnt = page.getNumTuples();
        int siblingCnt = sibling.getNumTuples();
        if (!isRightSibling) {
     ...
     * entries. Otherwise merge with one of the siblings. Update pointers as needed.
     *
     * @param tid        - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all
     *                   new dirty pages
    ...
    private void handleMinOccupancyInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        if (leftSiblingId != null) {
            BTreeInternalPage leftSibling = (BTreeInternalPage) getPage(tid, dirtypages,
                    leftSiblingId, Permissions.READ_WRITE);
            // if the left sibling is at minimum occupancy, merge with it. Otherwise
            ...
            BTreeInternalPage rightSibling = (BTreeInternalPage) getPage(tid, dirtypages,
                    rightSiblingId, Permissions.READ_WRITE);
            // if the right sibling is at minimum occupancy, merge with it. Otherwise
            // steal some entries from it
            if (rightSibling.getNumEmptySlots() >= maxEmptySlots) {
     ...
     * both pages are at least half full. Keys can be thought of as rotating through
     * the parent entry, so the original key in the parent is "pulled down" to the
     * right-hand page, and the last key in the left-hand page is "pushed up" to the
     * parent. Update parent pointers as needed.
     *
     * @param tid         - the transaction id
     * @param dirtypages  - the list of dirty pages which should be updated with all
     *                    new dirty pages
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        for (; pageCnt < siblingCnt; ++pageCnt, --siblingCnt) {
            ...
            BTreeEntry entry2 = new BTreeEntry(parentEntry.getKey(), entry.getRightChild(),
                    page.getChildId(0));
     ...
     * both pages are at least half full. Keys can be thought of as rotating through
     * the parent entry, so the original key in the parent is "pulled down" to the
     * left-hand page, and the last key in the right-hand page is "pushed up" to the
     * parent. Update parent pointers as needed.
     *
     * @param tid          - the transaction id
     ...
     * page. Delete the corresponding key and right child pointer from the parent,
     * and recursively handle the case when the parent gets below minimum occupancy.
    ...
    public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreePageId nextId = rightPage.getRightSiblingId();
        leftPage.setRightSiblingId(nextId);
        if (null != nextId) {
            BTreeLeafPage nextPage = (BTreeLeafPage) getPage(tid, dirtypages, nextId,
                    Permissions.READ_WRITE);
...
//        updateParentPointer(tid, dirtypages, parent.getId(), leftPage.getId()); //useless
     ...
     * and right child) from a parent node. If the parent becomes empty (no keys
     * remaining), that indicates that it was the root node and should be replaced
     * by its one remaining child. Otherwise, if it gets below minimum occupancy for
     * non-root internal nodes, it should steal from one of its siblings or merge
     * with a sibling.
     *
     * @param tid         - the transaction id
     * @param dirtypages  - the list of dirty pages which should be updated with all
    ...
    /**
     * Delete a tuple from this BTreeFile. May cause pages to merge or redistribute
     * entries/tuples if the pages become less than half full.
     *
     * @param tid - the transaction id
     * @param t   - the tuple to delete
     * @return a list of all pages that were dirtied by this operation. Could
     *         include many pages since parent pointers will need to be updated when
     *         an internal node merges.
     * @see #handleMinOccupancyPage(TransactionId, Map, BTreePage)
     */
    public List<Page> deleteTuple(TransactionId tid, Tuple t)
        ...
        BTreePageId pageId = new BTreePageId(tableid, t.getRecordId().getPageId().getPageNumber(),
                BTreePageId.LEAF);
        BTreeLeafPage page = (BTreeLeafPage) getPage(tid, dirtypages, pageId,
                Permissions.READ_WRITE);
        page.deleteTuple(t);
    ...
    /**
     * Get the page number of the first empty page in this BTreeFile. Creates a new
     * page if none of the existing pages are empty.
     *
     * @param tid        - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all
     *                   new dirty pages
    ...
    public int getEmptyPageNo(TransactionId tid, Map<PageId, Page> dirtypages)
        ...
        if (headerId != null) {
            ...
            while (headerPage != null && headerPage.getEmptySlot() == -1) {
                ...
                if (headerId != null) {
                    headerPage = (BTreeHeaderPage) getPage(tid, dirtypages, headerId,
                            Permissions.READ_ONLY);
                    headerPageCount++;
                } else {
                    headerPage = null;
     ...
     * if possible, and creates a new page if none are available. It wipes the page
     * on disk and in the cache and returns a clean copy locked with read-write
     * permission
     *
     * @param tid        - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all
    ...
    private Page getEmptyPage(TransactionId tid, Map<PageId, Page> dirtypages, int pgcateg)
        ...
        RandomAccessFile rf = new RandomAccessFile(f, "rw");
        rf.seek(BTreeRootPtrPage.getPageSize()
                + (long) (emptyPageNo - 1) * BufferPool.getPageSize());
     ...
     * the affected pages of the file, and may block until the lock can be acquired.
     *
     * @param tid   - the transaction id
     ...
     * method will acquire a read lock on the affected pages of the file, and may
...
class BTreeSearchIterator extends AbstractDbFileIterator {
     ...
     * matching the predicate or from the next page by following the right sibling
     * pointer.
     *
    ...
    protected Tuple readNext()
            throws TransactionAbortedException, DbException, NoSuchElementException {
        while (it != null) {
