...
实现了一个判环的死锁逻辑，具体而言，设计 `WaitTransaction` POJO 存 `getPage` 参数的哈希表 `waits`，记录所有正在等待获取锁的事务。设计 `DeadLockChecker` 维护 `waits` 和 `isDeadLock()`，使用 BFS 拓扑排序判断有向图是否有环，如果有，最后还有入度的任意点是环上的点，返回任意一个即可。做一个小模拟即可。其中需要使用 `pageLocks` 和 `waits` 建图。
...
该问题，后来发现是吞掉了 except 导致的，对 `xecution.Delete`，我的错误实现 `open` 方法为：
...
> 疑似有小概率不正确(可能哪里并发没处理好，比如原子性问题之类的)。
