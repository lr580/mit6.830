...
开一个常量池 `private Result[] blanks`，按 `Op what` 的枚举顺序 new 七个对象。然后根据 `what.ordinal()` 取枚举下标，利用 `clone` 办法构造新的初始值。如果此哈希表键从未见过，则将复制的常量池对象作 merge 左值 self，然后新 Tuple 跟他合并后返回 put 回去覆盖哈希表项。即可完成 `mergeTupleIntoGroup()` 实现。

对 `iterator()` 实现，将哈希表转 `List<Tuple>`(注意要将 `Result` 构造成 `Tuple`)，然后调用写好的 `storage.TupleIterator` 类转迭代器即可。

> 因为 `gbfield != NO_GROUPING` 的 if else 写反了(写成 ==)导致手动写 main debug 了十多分钟
