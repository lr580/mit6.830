...
<img width=400 src="lab3-controlflow.png">
...
非必须任务：需要通过单元测试 `BTreeDeadlockTest` 和单元测试 `BTreeNextKeyLockingTest`。最后通过 `BTreeTest` 系统测试，该测试可能需要花费一分钟。
...
对系统测试，缓存池每页大小设置成了 1024(默认4096)，创建一个 2 列(第一列主键) 31000 行的 B+ 树存储的数据表，缓存池页数修改为 500 页。接下来：

1. 开 200 个并发线程，间隔 100ms 启动一个，每个线程单独开一个事务，直接调用缓存池的 insert 插入一个随机 tuple。此时期望 B+ 树页面分裂。
2. 开 800 个并发线程，同时启动，事务同上。
3. 等待上面完成后，上面的 1000 个线程重新启动(重启操作的行值不一样，下同)，并启动 1000 个删除线程，每个线程单独删除一个随机的 tuple(如果找不到就不删，但肯定找得到)。
...
> 搞了半天，都没法弄完，一个 debug 如下，这里包含了有用的信息，其中数字是事务 id，并表明插入在等删除之前获得的读锁：
>
> ```
...
再次尝试，经过不知道多久可能半天以上的努力，分析出，删除事务获取了一个共享锁后，正常完毕事务，但没有释放掉这个锁。如：

```
Delete 1004
Delete step A 1004
Get page 1004 READ_ONLY (tableId: 512466940, pgNo: 0, pgcateg: ROOT_PTR)
Get page 1004 READ_ONLY (tableId: 512466940, pgNo: 253, pgcateg: INTERNAL)
Get page 1005 READ_ONLY (tableId: 512466940, pgNo: 0, pgcateg: ROOT_PTR)
Get page 1004 READ_ONLY (tableId: 512466940, pgNo: 252, pgcateg: INTERNAL)
Get page 1004 READ_ONLY (tableId: 512466940, pgNo: 180, pgcateg: LEAF)
Delete step A ing 1004
Get page 1004 READ_WRITE (tableId: 512466940, pgNo: 180, pgcateg: LEAF)
Delete step B 1004
Release lock of (tableId: 512466940, pgNo: 180, pgcateg: LEAF) by 1004
Delete Done 1004
1002 waits for 1004 (tableId: 512466940, pgNo: 253, pgcateg: INTERNAL) in READ_ONLY
```

分析 debug 可知：每个删除事务结束后，只释放了他自己的排他锁，没有释放共享锁。

进一步分析可知，阅读测试代码，事务顺利完成时调用的是：
...
而自己的前者方法只写了写回脏页和解除脏页排斥锁的代码，只在有两个参数的重载方法里才写了检测所有锁的释放代码。因为我误以为所有代码都是调用两参数重载方法，不会直接调用单参数重载方法。

修改：让单参数重载方法调用双参数重载方法，而不是反过来。

> 因为不断的 debug 和尝试，代码里可能有多次解除锁的冗余代码(即事务结束时对同一个锁尝试释放多次)，不影响，所以暂时忽略。

新的问题：后续的搜索测试要求进行 1w 次查找，从 `insertedTuples` 里查询，且全部查询成功，这就要求插入不能有事务失败。通过 debug 发现，具体有两个地方存在出错，其他地方都没事务失败：①800个同时启动的并发插入事务，大部分都失败了，且没有检测出死锁；②1000个增1000个删同时并发，删除事务偶尔(一百个平均有两个)失败，且每个失败都是死锁导致的。

仔细阅读日志，发现有几百个事务需要同时等待一个事务排他锁使用一个页面，等待了 2s 还是无法得到。那么有几种解决思路：①延长等待时间；②加快插入事务运行速度，使得在数秒内能执行完毕 800 个插入；③修改等待策略，如果发现要等待的东西变了(即事务还是在推动的，而不是一动不动)，就刷新等待，重新等。

尝试修改：考虑事务最大等待时间修改为 10s，如果 10s 还得不到锁就放弃。代价是 `Lab4.Exercise1` 的 `LockingTest` 每次都需要等这个时间才能结束。结果，从 800 个里失败了 749 个变成了失败。再次修改为 30s。至此，成功了，没有任何插入事务发生事务错误。然而，还是无法通过查询，具体而言，连续通过了 9 次(第二次跑通过了 90 次)然后有一次没通过，但是需要连续通过 1w 次，意味着全部插入一个都不能挂。

并且，尝试跳过搜索，直接进行 sanity check，也不过，这表明主键根本没有单调。

尝试只初始建树，直接跳到搜索和 sanity，能过。加上插入 200，能过。加上插入 800，不过。证明插入时有问题。相似地，将自己的 `BTreeFile` 扔到正确项目去跑能过，证明错不在 `BTreeFile` 而在 lab4 或之前。也试过把别人的 `BTreeFile` 正确扔到自己自己的项目，也不能过。尝试每次迭代遍历换页输出一下，发现新页开始跟旧页存在不单调情况。既然自己的插入逻辑没错，那只能是缓存池出错导致的单调性不一样。而这个不单调也可能会导致搜索出错。且出错的值总是被插入的值：

```jav
(5308, 28883)
(5393, 26488)
Next page (tableId: 891061919, pgNo: 433, pgcateg: LEAF)
(5308, 15756)
```

即，只要插入后被插入值是某个页面的最后一个元组，那么就可能出错。且反过来，所有出错的地方都一定是插入后在页面最后一个元组。而且在后续(全部事务都做)还有另一个问题，即遍历的数量对不上，会少一些元素。

> 不能理解为什么总是最后一个元组出错，所以无奈放弃了。

> 注意到他的插入事务逻辑是直接通过：
>
> ```java
> Tuple t = BTreeUtility.getBTreeTuple(tupdata);
> Database.getBufferPool().insertTuple(tid, bf.getId(), t); //bf是BTreeFile
> Database.getBufferPool().transactionComplete(tid);
> ```
>
> 且 `index.BTreeUtility` 有如下逻辑，即它是随机给了个 record id，并没有真的去执行 `BTreeFile` 的定位。
>
> ```java
...
> ```
>
> 但是在缓存池的 `insertTuple`，实际上是调用了 `getFile(tableId).insertTuple(tid, t)` 的，其逻辑是：找到要插入 tuple 所在叶页，如果没位了就先裂页，最后插进去。
...
Bonus。实现一个 `BTreeReverseScan` 类，逆序扫描 `BTreeFile`，以给定最优的 `IndexPredicate`。需要使用 `BTreeScan` 作为起始，可能需要实现一个 `BTreeFile` 类内 reverse iterator，或实现一个独立版本的 `BTreeFile.findLeafPage`。继续通过系统测试 `BTreeScanTest`。

> 具体而言，我发现自己的代码完全没动，能直接通过该测试，所以当已经完成了。
...
实现基于日志的回滚、恢复。`storage.LogFile` 类实现。

当一页读进来时，存储初始内容作为先前映像。事务更新一页时，记录更新后的映像。使用先前映像(before-image)来回滚丢弃事务，或者使用结果映像(after-image)去重做映像恢复。

再做页级锁前，需要做整页物理撤销实现。如果页被修改，一定有排他锁存在，即没有其他事务正在修改它，所以撤销只需要覆写整页即可。

将会测试 `BufferPool.flushAllPages()`，其中，需要对 `BufferPool.flushPage()`，在写页回盘前，对脏页事务 `tid`，执行：
...
功能是将更新日志写入磁盘。该方法已实现，不用管。

在完成了页面 flush 后，调用 `page.setBeforeImage()`。注意到，不能在 `flushPage()` 里调用，因为在事务尚未提交时，就进行 flush 了。所以通常建议在 `transactionComplete()` 或其他事务完成后的地方调用。

如果实现正确，此时可以通过 `LogTest` 系统测试代码顺序的前三个测试方法 `TestCommitCrash`, `TestFlushAll()`, `PatchTest()`。

实现：

- `public void rollback(TransactionId tid) throws NoSuchElementException, IOException`；会被事务 `Transaction` 执行失败时调用。操作磁盘，
- `public void recover() throws IOException`

前者在事务放弃(abort)时调用，在事务释放锁时调用。撤销对事务可能对数据库产生持久化影响的改动。应当读日志文件，找到所有相关事务的更新记录，获取先前映像，将其写到表文件里，使用 `raf.seek` 移动指针，`raf.readInt` 等读取数据，即直接读走而不赋值，用 `readPageData` 读映像。提供了 `tidToFirstLogRecord` map 来映射事务id 及其 heap file 偏移。保证已经将先前映像写回表文件的页面必须得到丢弃。可以使用 `print()` 方法测试。

注意到提供了 `private RandomAccessFile raf`，指向了日志文件。

在 `final Map<Long, Long> tidToFirstLogRecord` 提供了事务对应位置。看到提供的方法 `logXactionBegin()` 进行的操作，首先 `preAppend()` 对成员整数 `totalRecords` 自增，构造函数时设成员布尔值 `recoveryUndecided` 为真。若它是真，该方法将其设假，然后 `raf` 归零(`seek(0)`)，然后清空文件所有内容。先写一个常量 long `NO_CHECKPOINT_ID`，然后指针跳到它之后，更新成员属性当前指针 `long currentOffset`。 如果调用 `recover`，则 `recoveryUndecided` 置 false。

继续解析 `logXactionBegin()`，在预处理 `preAppend()` 后，记录的格式为：写常量标记(都是 int) `BEGIN_RECORD`，接着写事务 id long，接着写当前指针，然后维护 map，更新指针。下面每个方法基本都要 `preAppend`，略。

同理，有 `logWrite(TransactionId tid, Page before, Page after)`，先写常量标记 `UPDATE_RECORD`，然后写事务 id，然后先后写 before, after 两个 image，并最后写当前指针并更新。

有检查点记录 `logCheckpoint()`，首先强制存盘(`force`)，清空缓存池(这个过程会导致写盘进而写日志)，日志写常量标记 `CHECKPOINT_RECORD` 和 -1 空事务，接着写 map 的元素数(int)，将 map 写入磁盘，将其 key, value 逐个写盘。在这之后，到文件头部将 ``NO_CHECKPOINT_ID`` 或旧检查点指针位置改为当前 `CHECKPOINT_RECORD` 前的位置；当前记录段的最后写上结束位置的下标。

每次写检查点结束后，会调用 `logTruncate`，具体而言，从最新检查点开始读 map，求最早位置。之前的都从日志删除，之后的，有什么写什么。

实现完毕后，预期可以通过 `TestAbort, TestAbortCommitInterleaved` 这两个测试子方法。具体为：

1. `TestAbortCommitInterleaved`。
   - 首先对空数据表 `hf1`，启动事务，插入一行值 1，然后数据库 flush all，然后再插入一行值 2，然后提交事务。
   - 接下来，启动事务 `t1`，开启该事务，然后插入一行值 3
   - 启动事务 `t2`，开启该事务，然后对另一张表插入一行值 21
   - 记录检查点
   - `t2` 再插入一行值 22，`t2` 提交；`t1` 再插入一行值 4，`t1` 中断。逻辑为：清缓存池 `flushAllPages`，然后写 abort 日志，再清一次，然后调该事务失败
   - 模拟崩溃，直接重新实例化缓存池等，然后读盘取数据，调用 `recover()` 方法
   - 启动事务，搜索指定值，要求 1,2,21,22 找得到，且 3,4 找不到。
2. `TestAbort`
   - 对空数据表 `hf1`，同上理，插入一行 1，flush all，插入一行 2
   - 插入一行 4，然后 abort 事务，具体而言清空缓存池，写日志，再清空，然后调用缓存池的事务失败
   - 查找 1,2,3,4，预期仅前两个查得到

#### 具体实现

> 进行了从头重构，重构前思路：
>
> 类比 `logXactionBegin`，可知，首先要获取之前存好的 map 指针，将文件重定向到这里去。然后不断按格式读取。
>
> 根据上文分析，我们并不知道事务有多长，所以读到不能读(即 catch)为止，在这个过程中，不断读标记(类型常量int和事务id)，如果类型是 `UPDATE_RECORD`，读 before, after，如果事务 ID 与当前事务一致，因为是回滚，所以将 before 整页写进去(直接调 `DbFile`，不需要搞缓存池)(遍历后倒序写)；如果是检查点 `CHECKPOINT_RECORD`，整个读走跳过。其他常量不管。对其他类型，统一是读常量 int，事务 id long 和指针位置 long 即可。
>
> 写完了，一跑，没过任何一个新的系统测试，用正确代码测试能过，证明错误的地方就在新写的地方而不是之前的地方。
>
> 试着让 GPT4 对比了一下，发现每段记录后要读的一个指针位置long，我写成了之后 else if 读，把它挪出一个大括号即可。修改完毕，通过了对应的单元测试。
>
> > 执行完毕，根目录会出现临时文件，记得删了。
>
> 发现 `TestCommitCrash` 不过了，该过的两个确实过了。
>
> 如果没有实现 `rollback`，则对 `TestAbort` 发现能查到 4，这是因为 abort 前修改存盘了。对 `TestAbortCommitInterleaved` 查询 3 同理。
...
若数据库崩溃(crash)然后重启，使用 `recover()` 方法，在任何新事务启动之前进行恢复。需要：

1. 读取最后的检查点
2. 往前扫描检查点，找到所有执行失败中断而需要撤销的事务(loser)
3. 将这些事务全部撤销

`logCommit()` 方法表示了一个事务的成功结束， 格式为 `COMMIT_RECORD` 常量 int 标记+事务id+当前指针位置。且移除 map，更新指针等。

`logAbort()` 方法表示了一个事务的失败结束，格式为 `ABORT_RECORD` 常量和同上。

撤销的事务统统撤销，成功的事物统统重做，所以遍历时找到所有成功的事务，其他事务就是失败的事务。

首先重新明确，map 里的事务的含义是，所有执行到一半没做完的事务，他们可能正在写盘且写了一半但崩溃了，不知道写了多少，所以要处理掉。

根据代码逻辑可知，在写盘时，先写日志到盘，后写数据到盘；且只要不写盘，就不会触发写 update 日记。且 `logCheckpoint` 会强制将缓存池全部页面清空并写盘进而写日志。

根据 `transaction.Transaction` 代码可知，开启事务时写日志，成败提交事务也都写对应日记，成功时在数据写盘后写 commit 日志；失败时在事务前写日志。

对每个 map 的事务，根据定义一定是在检查点前开始，在检查点后结束的成功事务。

通过 `LogTest` 系统测试的全部测试方法。一些方法：

- `TestOpenCommitCheckpointOpenCrash`
  - 开事务，插表1值1，清缓存，插表1值2，提交
  - 事务 t1 插表1值12，清缓存，插表1值13
  - 开事务，插表2值26，清缓存，插表2值27，提交
  - 记录检查点
  - 开事务 t3，插表2值28，清缓存，插表2值29
  - 数据库崩溃，重置缓存池，执行 recover
  - 查询 1,26,27 找得到，12,13,22,23,24,25,28,29 找不到
- `TestOpenCommitOpenCrash`
  - 开事务，插表1值1，清缓存，插表1值2，提交
  - 事务 t1 插表1值10，清缓存，插表1值11
  - 开事务，插表2值22，清缓存，插表2值23，提交
  - 事务 t3 插表2值24，清缓存，插表2值24
  - 数据库崩溃，重置缓存池，执行 recover
  - 查询 1,22,23 找得到，10,11,24,25 找不到
...
> 根据阅读日志逻辑可知，需要做的事情有：
>
> 1. 检查点前开始，检查点后 abort 的 map 内事务，一步步撤销。可以对每个事务按文件顺序栈读 check point 前的操作，然后逆序写旧数据回盘。直接按时间序逆序写回即可。
> 2. 检查点前开始，检查点后 commit 的 map 内事务，从检查点后直接执行，直到结束。
>
> 首先指针回到开始，读检查点并构建 map。继续往后读，读检查点后的全部 `COMMIT_RECORD` 事务，记录成表。其余事务都是失败事务。
>
> 接下来，重新回到检查点后，对每一个更新，如果发现后面 commit 了，就写盘。所有其他情况忽略不计。
>
> 写盘具体操作同上一个 exercise，即，直接调用 `DbFile` 的写磁盘，不用管任何缓存池内容。
>
> 发现系统测试未过，具体而言有两个方法未过，且 `TestAbortCommitInterleaved` 没过，这意味着 abort 未正确执行。修改 `common.Debug`，对 `log` 方法去掉 if，进行日志输出。只运行错误测试用例，得：
>



注意到，事务的修改是排他锁，且是页级变动，这意味着：不考虑检查点，日志里每个事务只会修改同一个页面一次。考虑检查点，则一个事务一个页面可能在检查点后重新开一次。

考虑如何在同时实现了检查点、回滚日记的情况下，完成上述方法。

先看看重构前、简化后的实现：

1. 回滚：对该事务，map 跳日志起点，顺序将旧页面写盘，不动缓存池。
2. 恢复：读检查点，并往后读得到全部 map 里 commit 事务，对这些事务，检查点后新页写盘。

没有通过 `TestAbortCommitInterleaved` 和 `TestOpenCommitCheckpointOpenCrash`。

重新梳理了逻辑，发现问题为：abort 事务没有得到 abort。于是，进行修改，增加逻辑：找到最早的 abort 事务的点，跳到那个文件指针去，从那里开始，一个个 abort 的旧页顺序写进去，然后这么修改后测试点几乎全炸了。所以撤回修改，先分析一下测试输出。

后来发现，这个逻辑其实几乎是全错的，之所以不断简化看到像是对的，是因为在跑另一个项目的单元测试，误以为是这个项目的。

对第一个失败测试，记录检查后，表1插3，表2插21；然后表11插4且回退，表插22且提交。查3查到了，预期查不到。根据旧逻辑，没有处理 abort，所以状态确实查得到。

看看重构前、简化前的代码的恢复逻辑：

1. 读检查点，构造 map，构造 commit set，对每个 commit 事务，跳到 map 位置，一直读到检查点前，将旧数据复原。按事务 $O(n^2)$ 一个一个来，而不是按时间顺序。然后对检查点后，abort 的也旧复原，commit 的新页刷新。

如果改成单循环刷旧页，会全寄。

所以只能老实分析错误原因了，对可能最接近正确的简化前代码，鉴定为，abort 没删掉插入的 3。

注意到，`flushAllPages` 对选中的页面，调用 `removePage`，进而调用 `flushPage`，写盘写日志。输出每次 flush，发现：

```
Insert 2 rows
Flush simpledb.storage.HeapPageId@c62ba7e2
Flush simpledb.storage.HeapPageId@8f3cae17
Flush simpledb.storage.HeapPageId@8f3cae17
Inserted 2 rows
Oops, checkpoint
Flush simpledb.storage.HeapPageId@8f3cae17
...
分析可知，roll back 滚多了一页。试着逆序滚写。能过该测试。

现在查看下一个不过的测试 `TestOpenCommitCheckpointOpenCrash`，要找 12 找得到，理应找不到。既然找得到，那说明把 t1 当成提交了，即，没有正确地回滚 hf1。同理，需要逆序滚。之后也顺利完成了该测试。

之后多了一个不过的测试 `TestOpenCommitOpenCrash`，找 10 找得到，也是证明当提交了。但是发现，map 压根没有数据。注意到数据库崩溃会重启日志，然后调用 recover，这时候比较麻烦，需要想办法把 map 建出来。

然后添加了建 map 代码，对没有检查点时，扫描全部日记，发现 begin 建一个 map，发现 commit 删一个。然后不能过多了一个 `TestOpenCrash`。

简单阅读和分析和 debug，发现原因在于没有检查点时不会写 old，因为判断指针小于检查点时才写，而检查点不存在，所以手动设置检查点为无穷值。支持，这两个过了，但是 `TestCommitAbortCommitCrash` 不过了。第二次查 7 查不到。

由于 abort，导致数据恢复到了写 7 前，即这个 abort 顶替掉了。修复：将 abort 同 commit 处理，因为 abort 在其它代码得到了正确处理，所以从 map 移除。至此，通过全部测试。



重新梳理一下实现 `LogFile` 两个方法的逻辑：

1. `rollback` 方法会在 `logAbort` 方法时调用，在写 `ABORT_RECORD` 前调用。`rollback` 将从日志未完成事务集合 map 找该事务起始点开始读，读到每个页面(测试方法里有强制 flush all 打破 no steal，所以不一定每个页面只更新一次)，记录每个旧页，然后逆序将每个旧页写盘

2. `recover` 方法将在模拟数据库崩溃并重启后执行。具体而言，如果有检查点，首先读取检查点并构造 map；否则，从日志开始读，读到 begin 就插入 map，读到 commit/abort 就删除 map。对 abort，这是因为已经 rollback 过了，所以不需要再第二次处理，这里只会记录执行到一半尚未知道 commit/abort 与否的事务。并且，将检查点设为无穷大。

   然后，读取检查点往后的日志，记录在这之后 commit 了的事务(如果没有检查点，跳过这一步)。然后遍历map，对每个不在 commit 在 map 的事务，先按日志顺序读取该事务的全部旧页，然后逆序写回旧页。一个一个事务地分别处理，不要一起处理。这样的话，若有检查点，检查点后 abort 的和尚未提交的，都会被清理。

   然后对检查点后的 commit，将之后的新更新全部写盘，直接按序遍历即可。



至此，完成了所有必须完成的 exercise，下面检测是否通过了所有单元测试和系统测试。在 eclipse 内右击一个包，run as - junit test 执行该包的全部 test 方法。通过了所有单元测试、除不必须完成的 `BTreeTest` 外的全部系统测试。因此，可以认为顺利地基本完成了整个项目。
