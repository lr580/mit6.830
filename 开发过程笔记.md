...
根据上文分析，我们并不知道事务有多长，所以读到不能读(即 catch)为止，在这个过程中，不断读标记(类型常量int和事务id)，如果类型是 `UPDATE_RECORD`，读 before, after，如果事务 ID 与当前事务一致，因为是回滚，所以将 before 整页写进去(直接调 `DbFile`，不需要搞缓存池)；如果是检查点 `CHECKPOINT_RECORD`，整个读走跳过。其他常量不管。对其他类型，统一是读常量 int，事务 id long 和指针位置 long 即可。
...
根据代码逻辑可知，在写盘时，先写日志到盘，后写数据到盘；且只要不写盘，就不会触发写 update 日记。这意味着：失败事务绝对不会写盘。根据 `transaction.Transaction` 代码可知，开启事务时写日志，成败提交事务也都写对应日记，成功时在数据写盘后写 commit 日志；失败时在事务前写日志。
...
首先指针回到开始，读检查点并跳过检查点，然后读检查点后的全部 `COMMIT_RECORD` 事务，记录成表。
