...
如果实现正确，此时可以通过 `LogTest` 系统测试代码顺序的前三个测试方法。
...
有检查点记录 `logCheckpoint()`，首先强制存盘(`force`)，清空缓存池，日志写常量标记 `CHECKPOINT_RECORD` 和 -1 空事务，接着写 map 的元素数(int)，将 map 写入磁盘，将其 key, value 逐个写盘。在这之后，到文件头部将 ``NO_CHECKPOINT_ID`` 或旧检查点指针位置改为当前 `CHECKPOINT_RECORD` 前的位置；当前记录段的最后写上结束位置的下标。

实现完毕后，预期可以通过 `TestAbort, TestAbortCommitInterleaves` 这两个测试子方法。

#### 具体实现

类比 `logXactionBegin`，可知，首先要获取之前存好的 map 指针，将文件重定向到这里去。然后不断按格式读取。

根据上文分析，我们并不知道事务有多长，所以读到不能读(即 catch)为止，在这个过程中，不断读标记(类型常量int和事务id)，如果类型是 `UPDATE_RECORD`，读 before, after，如果事务 ID 与当前事务一致，因为是回滚，所以将 before 整页写进去(直接调 `DbFile`，不需要搞缓存池)，然后缓存池把这一页丢掉；如果是检查点 `CHECKPOINT_RECORD`，整个读走跳过。其他常量不管。对其他类型，统一是读常量 int，事务 id long 和指针位置 long 即可。
...
通过 `LogTest` 系统测试的全部测试方法。

#### 具体实现

首先指针回到开始，读检查点，重构 map(局部即可，可以不要换掉全局的)，如果有检查点，用检查点构建 map。

撤销的事务统统撤销，成功的事物统统重做，所以遍历时找到所有成功的事务，其他事务就是失败的事务。

首先重新明确，map 里的事务的含义是，所有执行到一半没做完的事务，他们可能正在写盘且写了一半但崩溃了，不知道写了多少，所以要处理掉。

根据代码逻辑可知，在事务提交时，先写日志到盘，后写数据到盘。故发生崩溃后，

规定在 commit 前，所有更新不写盘，所以检查点前的失败事务全部不管。且根据规定，所有成功事务需要重做。

复原 map 里的全部数据，对每个 map 的事务，如果提交成功了(具体而言，在检查点前开始，在检查点后结束)，那么对这个事务的，在检查点之前的全部操作(具体为每个页面结果)，悉数从先到后撤销，这样的顺序操作，将会使得，对同一个页面的多次变动，回复检查点(崩溃前)到最后一次变动成功前的状态。因为日志可能很长，一个经验假设是未完成事务总是很靠近检查点，所以对每个事务，直接用 map 跳到检查前的开始地方，然后开始小范围地走到检查点。可以取 map 里 min 指针优化，这里从略。
